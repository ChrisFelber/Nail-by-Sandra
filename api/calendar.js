...
function availableSlots(date, durationMinutes, busy) {return SLOT_STARTS.filter(time => {const start = zonedLocalToDate(date, time);const end = addMinutes(start, durationMinutes);return !overlapsBusy(start, end, busy);});}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      return json(res, 200, {date,timeZone: TIME_ZONE,busy,available: availableSlots(date, durationMinutes, busy)});
    }
    ...
    const busy = await getBusy(token, calendarId, date);
    if (overlapsBusy(start, end, busy)) return json(res, 409, { error: 'Ce créneau vient d’être réservé. Merci d’en choisir un autre.', code: 'SLOT_UNAVAILABLE' });
    ...
    try {
      const message=[
        ...
        '',
        'Le rendez-vous a été ajouté à ton calendrier.'
      ].filter(Boolean).join('\n');
      await sendGmailToSelf({subject:`Nouveau rendez-vous — ${customer.firstName} ${customer.lastName}`,body:message});
      notificationSent=true;
    } catch (emailError) {
      console.error('Booking notification email:', emailError);
    }
