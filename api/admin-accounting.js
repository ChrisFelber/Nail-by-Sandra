...
export default async function handler(req,res){
 if(!isAuthenticated(req))return json(res,401,{error:'Non autorisé.'});const sql=getDb();
 try{
  if(req.method==='POST'){
   ...
   if(action==='category'){
    const name=String(req.body?.name||'').trim().slice(0,80);if(!name)return json(res,400,{error:'Nom de catégorie requis.'});
    const rows=await sql`INSERT INTO expense_categories(name) VALUES(${name}) ON CONFLICT(name) DO UPDATE SET active=TRUE RETURNING id,name,active`;return json(res,201,{ok:true,category:rows[0]});
   }
   if(action==='appointment_update'){
    const id=Number(req.body?.id),reason=String(req.body?.reason||'').trim().slice(0,300),service=String(req.body?.service_name||'').trim().slice(0,120),amount=Number(req.body?.service_amount_cents);
    ...
    const before=await snapshot(sql,id);if(!before)return json(res,404,{error:'Prestation introuvable.'});
    const manual=(before.items||[]).filter(i=>i.source==='manual').reduce((s,i)=>s+Number(i.amount_cents||0)*Number(i.quantity||1),0),total=amount+manual;
    await sql.transaction([sql`UPDATE appointments_accounting SET booked_service_name=${service},booked_service_amount_cents=${amount},total_amount_cents=${total},updated_at=NOW() WHERE id=${id}`,sql`UPDATE appointment_items SET item_name=${service},amount_cents=${amount} WHERE appointment_accounting_id=${id} AND source='booked'`]);
    const after=await snapshot(sql,id);await sql`INSERT INTO accounting_audit_log(appointment_accounting_id,action,reason,before_data,after_data) VALUES(${id},'corrected',${reason},${JSON.stringify(before)}::jsonb,${JSON.stringify(after)}::jsonb)`;return json(res,200,{ok:true,appointment:after});
   }
   if(action==='appointment_reverse'){
    const id=Number(req.body?.id),reason=String(req.body?.reason||'').trim().slice(0,300);if(!Number.isInteger(id)||id<=0||!reason)return json(res,400,{error:'Motif d’annulation requis.'});
    const before=await snapshot(sql,id);if(!before)return json(res,404,{error:'Prestation introuvable.'});await sql`UPDATE appointments_accounting SET accounting_status='reversed',reversed_at=NOW(),updated_at=NOW() WHERE id=${id}`;const after=await snapshot(sql,id);await sql`INSERT INTO accounting_audit_log(appointment_accounting_id,action,reason,before_data,after_data) VALUES(${id},'reversed',${reason},${JSON.stringify(before)}::jsonb,${JSON.stringify(after)}::jsonb)`;return json(res,200,{ok:true});
   }
  ...
  if(req.method==='GET'){
   ...
   const [appointments,expenses,categories,audit,annualRows]=await sql.transaction([
    ...
    sql`SELECT id,expense_date,category,description,amount_cents,receipt_url,receipt_name,created_at FROM expenses WHERE expense_date>=${mb.start}::date AND expense_date<${mb.nextMonth}::date ORDER BY expense_date DESC,id DESC`,
    sql`SELECT id,name FROM expense_categories WHERE active=TRUE ORDER BY name`,
    sql`SELECT id,appointment_accounting_id,action,reason,created_at FROM accounting_audit_log ORDER BY created_at DESC LIMIT 50`,
    sql`SELECT 'revenue' type,COALESCE(SUM(total_amount_cents),0)::bigint total FROM appointments_accounting WHERE accounting_status='confirmed' AND appointment_date>=${yb.start}::date AND appointment_date<${yb.nextYear}::date UNION ALL SELECT 'expense',COALESCE(SUM(amount_cents),0)::bigint FROM expenses WHERE expense_date>=${yb.start}::date AND expense_date<${yb.nextYear}::date`
